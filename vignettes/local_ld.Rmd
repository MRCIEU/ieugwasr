...
vignette: >
  %\VignetteIndexEntry{Running local LD operations}
...
```
ld_clump(
    dplyr::tibble(variant=dat$rsid, pval=dat$pval, id=dat$trait_id),
...
To automatically extract variants from a dataset, and search for LD proxies when a requested variant is not present in the dataset, please look at the options available in the [gwasvcf](https://mrcieu.github.io/gwasvcf/) package:
