---
title: Comparison if rsid vs chr:pos lookup in ElasticSearch
...
This is a `r radius * 2` size window, which has `r length(rsid)` variants to lookup.
...
This `r length(chrpos)` ranges of `r 20000` size window, which is covered by `r length(rsid)` variants to lookup.
